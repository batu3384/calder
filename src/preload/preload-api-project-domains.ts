  ...
  return {
    context: {
      getProjectState: (projectPath: string) =>
        ipcRenderer.invoke('context:getProjectState', projectPath),
      createStarterFiles: (projectPath: string) =>
        ipcRenderer.invoke('context:createStarterFiles', projectPath),
      ...
      onChanged: (callback) =>
        onChannel('context:changed', (projectPath, state) =>
          callback(projectPath as string, state as ProjectContextState),
        ),
    },
    workflow: {
      getProjectState: (projectPath: string) =>
        ipcRenderer.invoke('workflow:getProjectState', projectPath),
      createStarterFiles: (projectPath: string) =>
        ipcRenderer.invoke('workflow:createStarterFiles', projectPath),
      createFile: (projectPath: string, title: string) =>
        ipcRenderer.invoke('workflow:createFile', projectPath, title),
      readFile: (projectPath: string, workflowPath: string) =>
        ipcRenderer.invoke('workflow:readFile', projectPath, workflowPath),
      watchProject: (projectPath: string) => ipcRenderer.send('workflow:watchProject', projectPath),
      onChanged: (callback) =>
        onChannel('workflow:changed', (projectPath, state) =>
          callback(projectPath as string, state as ProjectWorkflowState),
        ),
    },
    teamContext: {
      getProjectState: (projectPath: string) =>
        ipcRenderer.invoke('teamContext:getProjectState', projectPath),
      createStarterFiles: (projectPath: string) =>
        ipcRenderer.invoke('teamContext:createStarterFiles', projectPath),
      createSpace: (projectPath: string, title: string) =>
        ipcRenderer.invoke('teamContext:createSpace', projectPath, title),
      watchProject: (projectPath: string) =>
        ipcRenderer.send('teamContext:watchProject', projectPath),
      onChanged: (callback) =>
        onChannel('teamContext:changed', (projectPath, state) =>
          callback(projectPath as string, state as ProjectTeamContextState),
        ),
    },
    review: {
      getProjectState: (projectPath: string) =>
        ipcRenderer.invoke('review:getProjectState', projectPath),
      createFile: (projectPath: string, title: string) =>
        ipcRenderer.invoke('review:createFile', projectPath, title),
      readFile: (projectPath: string, reviewPath: string) =>
        ipcRenderer.invoke('review:readFile', projectPath, reviewPath),
      watchProject: (projectPath: string) => ipcRenderer.send('review:watchProject', projectPath),
      onChanged: (callback) =>
        onChannel('review:changed', (projectPath, state) =>
          callback(projectPath as string, state as ProjectReviewState),
        ),
    },
    governance: {
      getProjectState: (projectPath: string, sessionId?: string) =>
        ipcRenderer.invoke('governance:getProjectState', projectPath, sessionId),
      setAutoApprovalMode: (
        projectPath: string,
        scope: 'global' | 'project',
        mode: AutoApprovalMode | null,
      ...
      createStarterPolicy: (projectPath: string) =>
        ipcRenderer.invoke('governance:createStarterPolicy', projectPath),
      watchProject: (projectPath: string) =>
        ipcRenderer.send('governance:watchProject', projectPath),
      onChanged: (callback) =>
        onChannel('governance:changed', (projectPath, state) =>
          callback(projectPath as string, state as ProjectGovernanceState),
        ),
    },
    task: {
      getProjectState: (projectPath: string) =>
        ipcRenderer.invoke('task:getProjectState', projectPath),
      create: (projectPath: string, title: string, prompt: string) =>
        ipcRenderer.invoke('task:create', projectPath, title, prompt),
      read: (projectPath: string, taskPath: string) =>
        ipcRenderer.invoke('task:read', projectPath, taskPath),
      watchProject: (projectPath: string) => ipcRenderer.send('task:watchProject', projectPath),
      onChanged: (callback) =>
        onChannel('task:changed', (projectPath, state) =>
          callback(projectPath as string, state as ProjectBackgroundTaskState),
        ),
    },
    checkpoint: {
      getProjectState: (projectPath: string) =>
        ipcRenderer.invoke('checkpoint:getProjectState', projectPath),
      create: (projectPath: string, snapshot: ProjectCheckpointSnapshotInput) =>
        ipcRenderer.invoke('checkpoint:create', projectPath, snapshot),
      read: (projectPath: string, checkpointPath: string) =>
        ipcRenderer.invoke('checkpoint:read', projectPath, checkpointPath),
      watchProject: (projectPath: string) =>
        ipcRenderer.send('checkpoint:watchProject', projectPath),
      onChanged: (callback) =>
        onChannel('checkpoint:changed', (projectPath, state) =>
