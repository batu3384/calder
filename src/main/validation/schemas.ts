...
export const CliProviderCapabilitySchema = z.object({
  ...
  pendingPromptTrigger: z.union([z.literal('startup-arg'), z.literal('env-var'), z.literal('none'), z.string()]),
...
export const FsReadFileSchema = z.object({
  ...
  maxBytes: z.number().int().positive().max(10 * 1024 * 1024).default(10 * 1024 * 1024), // 10MB default
});

export const StoreSaveSchema = z.object({
  state: z.unknown(),
  maxBytes: z.number().int().positive().max(25 * 1024 * 1024).default(25 * 1024 * 1024),
});
 ...
 */
export function validate<T>(schema: z.ZodType<T>, data: unknown): { ok: true; data: T } | { ok: false; errors: string[] } {
