import fs from 'node:fs';
import path from 'node:path';
import type { ProjectWorkflowSource, ProjectWorkflowState } from '../../shared/types.js';

function isFile(filePath: string): boolean {
  try {
    return fs.statSync(filePath).isFile();
  } catch {
    return false;
  }
}

function listMarkdownFiles(dirPath: string): string[] {
  try {
    return fs.readdirSync(dirPath)
      .filter((entry) => entry.endsWith('.md'))
      .sort((left, right) => left.localeCompare(right));
  } catch {
    return [];
  }
}

function readSummary(filePath: string): string {
  try {
    const contents = fs.readFileSync(filePath, 'utf8');
    const lines = contents
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean);
    const candidate = lines[0] ?? '';
    return candidate.startsWith('#') ? candidate.replace(/^#+\s*/, '').trim() : candidate;
  } catch {
    return '';
  }
}

function buildWorkflow(filePath: string): ProjectWorkflowSource {
  const stat = fs.statSync(filePath);
  return {
    id: `workflow:${filePath}`,
    path: filePath,
    displayName: path.basename(filePath),
    summary: readSummary(filePath),
    lastUpdated: new Date(stat.mtimeMs).toISOString(),
  };
}

export async function discoverProjectWorkflows(projectPath: string): Promise<ProjectWorkflowState> {
  const workflowDir = path.join(projectPath, '.calder', 'workflows');
  const workflows = listMarkdownFiles(workflowDir)
    .map((entry) => path.join(workflowDir, entry))
    .filter(isFile)
    .map((filePath) => buildWorkflow(filePath));

  const lastUpdated = workflows
    .map((workflow) => workflow.lastUpdated)
    .sort()
    .at(-1);

  return {
    workflows,
    lastUpdated,
  };
}
