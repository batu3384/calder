...
/** Mock .calderignore auto-creation: writeFileSync captures content, readFileSync returns it after creation. */
function mockCalderignoreAutoCreate(): void {
  let calderignoreContent: string | null = null;
  mockFs.writeFileSync.mockImplementation((_p: fs.PathOrFileDescriptor, data: string | NodeJS.ArrayBufferView) => {
    if (String(_p).endsWith('.calderignore')) calderignoreContent = String(data);
  });
  mockFs.readFileSync.mockImplementation((p: fs.PathOrFileDescriptor) => {
    if (String(p).endsWith('.calderignore') && calderignoreContent) return calderignoreContent;
...
describe('genericContextProducer', () => {
  ...
  it('creates .calderignore with default patterns when it does not exist', () => {
    mockFs.statSync.mockImplementation(() => { throw new Error('ENOENT'); });
    mockCalderignoreAutoCreate();
    ...
    expect(mockFs.writeFileSync).toHaveBeenCalledWith(
      expect.stringContaining('.calderignore'),
  ...
  it('does not overwrite existing .calderignore', () => {
    mockFs.readFileSync.mockImplementation((p: fs.PathOrFileDescriptor) => {
      if (String(p).endsWith('.calderignore')) return 'custom-pattern.ts\n';
      throw new Error('ENOENT');
    });
    mockFs.statSync.mockImplementation((p: fs.PathLike) => {
      if (String(p).endsWith('.calderignore')) return { isFile: () => true } as fs.Stats;
      throw new Error('ENOENT');
    });
  ...
  it('detects large files', () => {
    ...
    expect(check.description).toContain('.calderignore');
  });
  ...
  it('ignores package-lock.json via auto-created .calderignore defaults', () => {
    mockFs.statSync.mockImplementation(() => { throw new Error('ENOENT'); });
    mockCalderignoreAutoCreate();
  ...
  it('applies custom .calderignore patterns', () => {
    mockFs.readFileSync.mockImplementation((p: fs.PathOrFileDescriptor) => {
      if (String(p).endsWith('.calderignore')) return 'generated-data.json\n';
      throw new Error('ENOENT');
    });
    mockFs.statSync.mockImplementation((p: fs.PathLike) => {
      if (String(p).endsWith('.calderignore')) return { isFile: () => true } as fs.Stats;
