...
import { getPtyCwd, hasPtySession, isSilencedExit, killPty, resizePty, spawnPty, spawnShellPty, writePty } from './pty-manager';
import {
...
export function registerPtyIpcHandlers(ops: PtyIpcOps): void {
  ipcMain.handle('pty:create', async (_event, sessionId: string, cwd: string, cliSessionId: string | null, isResume: boolean, extraArgs: string, providerId: ProviderId = 'claude', initialPrompt?: string) => {
    ...
    if (entry && now < entry.resetAt) {
      ...
      ptyCreateRateLimit.set(payload.sessionId, { count: 1, resetAt: now + RATE_LIMIT_WINDOW_MS });
    }
