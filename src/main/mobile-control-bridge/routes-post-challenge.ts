...
export function createChallengePostHandler(
  ...
  return async function handleChallengeRequest(
    ...
    if (
      failRateLimited(
        ...
        'challenge',
        config.rateLimit,
        copy.serverMessage.tooManyChallengeRequests,
      )
    )
      return;

    const body = await parsePairingBody<{ token?: unknown; challenge?: unknown }>(
      record,
      req,
      res,
      config.maxBodyBytes,
