...
describe('installCodexHooks', () => {
  ...
  it('all hook commands reference $CALDER_SESSION_ID', () => {
    ...
    for (const [, matchers] of Object.entries(hooks) as [string, any[]][]) {
      for (const matcher of matchers) {
        for (const h of matcher.hooks) {
          expect(h.command).toContain('CALDER_SESSION_ID');
        }
      }
    }
  });
...
describe('validateCodexHooks', () => {
  ...
  it('does not treat unknown legacy hooks as installed', () => {
    ...
    mockFiles({
      [CONFIG_TOML]: '[features]\ncodex_hooks = true\n',
      [HOOKS_JSON]: JSON.stringify(legacyOnly),
    });

    const result = validateCodexHooks();
    expect(result.hooks).toBe('missing');
    expect(result.hookDetails.SessionStart).toBe(false);
    expect(result.hookDetails.UserPromptSubmit).toBe(false);
    expect(result.hookDetails.PostToolUse).toBe(false);
    expect(result.hookDetails.Stop).toBe(false);
    expect(result.hookDetails.PermissionRequest).toBe(false);
  });
...
describe('cleanupCodexHooks', () => {
  ...
  it('removes hooks key when all hooks are calder hooks', () => {
    const existing = {
      version: '1.0',
      hooks: {
        SessionStart: [
