...
describe('RendererPersistQueue', () => {
  it('retries the latest snapshot after a failed save', async () => {
    ...
    queue.enqueue({ version: 1, projects: [{ id: 'p1' }], activeProjectId: 'p1', preferences: {} } as never);

    await vi.waitFor(() => expect(save).toHaveBeenCalled());
    await vi.waitFor(() => {
      const savedLatest = save.mock.calls.some(
        (call) => call[0]?.activeProjectId === 'p1' && save.mock.results[save.mock.calls.length - 1]?.type !== 'throw',
      );
