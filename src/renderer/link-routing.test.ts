...
describe('resolveNavigableHttpUrl', () => {
  it('normalizes bare localhost links', () => {
    expect(resolveNavigableHttpUrl('localhost:3000/dashboard')).toBe(
      'http://localhost:3000/dashboard',
    );
  });

  it('extracts the target from markdown links', () => {
    expect(resolveNavigableHttpUrl('[Open](http://localhost:3000/tickets)')).toBe(
      'http://localhost:3000/tickets',
    );
  });

  it('removes wrapper punctuation around links', () => {
    expect(resolveNavigableHttpUrl('("http://localhost:3000/admin/dashboard"),')).toBe(
      'http://localhost:3000/admin/dashboard',
    );
  });

  it('strips ansi escape noise from link payloads', () => {
    expect(resolveNavigableHttpUrl('\u001B[32mhttp://localhost:3000/admin/tickets\u001B[0m')).toBe(
