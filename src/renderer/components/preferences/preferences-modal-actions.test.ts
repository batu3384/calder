...
const actionsSource = readFileSync(new URL('./preferences-modal-actions.ts', import.meta.url), 'utf-8');
...
describe('preferences modal theme preview contract', () => {
  ...
  it('captures saved theme when modal opens', () => {
    ...
    expect(modalSource).toContain('revertPreview: () => applyAppearanceTheme(savedAppearanceTheme)');
  });
});
