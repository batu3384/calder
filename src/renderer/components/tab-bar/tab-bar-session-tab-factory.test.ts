...
describe('tab bar session tab factory extraction', () => {
  ...
  it('keeps session tab badges, activation, context menu, and reorder behavior in helper module', () => {
    ...
    expect(sessionTabFactorySource).toContain("buildProviderIconMarkup(providerId, hasMultipleAvailableProviders())");
    expect(sessionTabFactorySource).toContain('options.showTabContextMenu(event.clientX, event.clientY, project, session, tab)');
    expect(sessionTabFactorySource).toContain("event.dataTransfer!.setData('text/plain', session.id)");
    expect(sessionTabFactorySource).toContain('appState.reorderSession(project.id, draggedId, targetIndex)');
  });
});
