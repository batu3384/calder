...
const mobileControlSource = readFileSync(
  new URL('./tab-bar/tab-bar-mobile-control.ts', import.meta.url),
  'utf-8',
);
...
const shareDialogCoreSource = readFileSync(
  new URL('./share-dialog/share-dialog.ts', import.meta.url),
  'utf-8',
);
const shareDialogCopySource = readFileSync(
  new URL('./share-dialog/share-dialog-copy.ts', import.meta.url),
  'utf-8',
);
...
describe('mobile control discoverability contract', () => {
  ...
  it('explains when mobile QR handoff becomes available inside share dialog', () => {
    expect(shareDialogSource).toContain(
      'Start Sharing to generate a secure mobile QR and one-time code.',
    );
  });
});
