...
export function getShareDialogCopy(language: ShareDialogLanguage): ShareDialogCopy {
  if (language === 'tr') {
    return {
      ...
      heroCopy:
        'Güvenli bir eşler arası devir başlatın, erişim seviyesini seçin ve diğer kişiyi bağlantı akışında yönlendirin.',
      ...
      mobileConnectionMetaConnected: (sessionName, modeLabel, durationLabel) =>
        `Aktif oturum: ${sessionName} · Mod: ${modeLabel} · Süre: ${durationLabel}`,
      mobileConnectionMetaWaiting: 'Güvenli doğrulama bekleniyor...',
      sharedTeamContext: (spaces, rules, workflows) =>
        `Paylaşılan ekip bağlamı: ${spaces} alan · ${rules} paylaşılan kural · ${workflows} iş akışı.`,
      historyNotice: 'Terminal geçmişinizin tamamı karşı tarafla paylaşılacaktır.',
      readWriteWarning:
        'Okuma-yazma modu karşı tarafın terminalinize yazmasına ve komut çalıştırmasına izin verir. Sadece güvendiğiniz kişilerle paylaşın.',
      accessLevel: 'Erişim seviyesi',
      readOnly: 'Salt okunur',
      readWrite: 'Okuma-yazma',
      mobileDiscoverabilityNotice:
        "Güvenli mobil QR ve tek kullanımlık kod üretmek için Paylaşımı Başlat'a tıklayın.",
      ...
      mobileHandoffHint:
        'Mobil devir (QR + tek kullanımlık kod). Parola girmenize gerek yok; sadece QR + OTP yeterlidir.',
      ...
      mobileResponseValidationFailed:
        'Mobil yanıt alındı ancak doğrulanamadı. Manuel bağlanmayı deneyin.',
      ...
      mobileHandoffFailedWithReason: (reason) =>
        `Mobil devir başarısız: ${reason}. Manuel Kodları kullanın veya QR\'ı tekrar deneyin.`,
      mobileHandoffFailedFallback:
        "Mobil devir şu anda başarısız. Manuel Kodları kullanın veya QR'ı tekrar deneyin.",
      mobilePairingExpired:
        "Mobil eşleştirmenin süresi doldu. QR'ı tekrar deneyin veya Manuel Kodlarla devam edin.",
      mobilePairingCheckFailedRepeated:
        "Mobil eşleştirme kontrolü art arda başarısız oldu. Manuel Kodlarla devam edin veya QR'ı tekrar deneyin.",
      mobilePairingCheckFailedRetrying:
        'Mobil eşleştirme kontrolü başarısız oldu. Otomatik olarak yeniden deneniyor…',
      ...
      authenticationFailedRestart:
        'Kimlik doğrulama başarısız. Yeni güvenli devir için paylaşımı yeniden başlatın.',
  ...
  return {
    ...
    heroCopy:
      'Open a secure peer-to-peer handoff, choose the access level, and guide the other person through the connection flow.',
    ...
    mobileConnectionMetaConnected: (sessionName, modeLabel, durationLabel) =>
      `Active session: ${sessionName} · Mode: ${modeLabel} · Duration: ${durationLabel}`,
    mobileConnectionMetaWaiting: 'Waiting for secure authentication...',
    sharedTeamContext: (spaces, rules, workflows) =>
      `Shared team context: ${spaces} spaces · ${rules} shared rules · ${workflows} workflows.`,
    historyNotice: 'Your full terminal scrollback history will be shared with the peer.',
    readWriteWarning:
      'Read-write mode allows the peer to type into your terminal and execute commands. Only share with people you trust.',
    ...
    mobileHandoffHint:
      'Mobile handoff (QR + one-time code). No passphrase entry is required on phone; QR + OTP is enough.',
    ...
    mobileResponseValidationFailed:
      'Mobile response was received but failed to validate. Try manual connect.',
    ...
    mobileHandoffFailedWithReason: (reason) =>
      `Mobile handoff failed: ${reason}. Use Manual Codes or retry QR.`,
    ...
    mobilePairingCheckFailedRepeated:
      'Mobile pairing check failed repeatedly. Continue with Manual Codes or retry QR.',
