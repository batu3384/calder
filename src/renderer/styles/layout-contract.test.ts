...
const browserPaneSource = readFileSync(
  new URL('../components/browser-tab/pane.ts', import.meta.url),
  'utf-8',
);
const browserPaneShellSource = readFileSync(
  new URL('../components/browser-tab/pane-shell.ts', import.meta.url),
  'utf-8',
);
...
describe('layout stylesheet contract', () => {
  it('imports the context inspector stylesheet and command deck selectors', () => {
    ...
    expect(browserPaneContractSource).toContain(
