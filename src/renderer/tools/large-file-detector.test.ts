...
describe('handleToolFailure', () => {
  ...
  // --- .calderignore exclusions ---

  it('does not alert for files matching .calderignore patterns', async () => {
    const projectId = setupProject();
    const session = appState.addSession(projectId, 'Session 1')!;
    const alerts: LargeFileAlert[] = [];
    onLargeFileAlert((alert) => alerts.push(alert));
  ...
  it('still alerts for files not matching .calderignore patterns', async () => {
    const projectId = setupProject();
    const session = appState.addSession(projectId, 'Session 1')!;
    const alerts: LargeFileAlert[] = [];
    onLargeFileAlert((alert) => alerts.push(alert));
  ...
  it('handles missing .calderignore gracefully', async () => {
    const projectId = setupProject();
    const session = appState.addSession(projectId, 'Session 1')!;
    const alerts: LargeFileAlert[] = [];
    onLargeFileAlert((alert) => alerts.push(alert));

    mockReadFile.mockResolvedValue({ ok: false, reason: 'not-found' });

    await handleToolFailure(session.id, makeReadFailure('/tmp/test/src/app.ts'));
    expect(alerts).toHaveLength(1);
  });

  it('caches .calderignore patterns per project', async () => {
    const projectId = setupProject();
    const session = appState.addSession(projectId, 'Session 1')!;
    const alerts: LargeFileAlert[] = [];
    onLargeFileAlert((alert) => alerts.push(alert));
